...
class DataLoader:
    def __init__(self, root_path, extensions=[], shuffle=True, seed=12071998):
        ...
        self.extensions = extensions  # if empty list, all file extensions are considered
        ...
        if self._shuffle:
            ...
            self.random_generator = random.Random(self._seed).sample(range(len(self.file_tuples)), len(self.file_tuples))
            self.random_index = 0
        ...
        for root, _, files in os.walk(self.root_path):
            ...
            for base_filename in base_filenames:
                if self.extensions:
                    ...
                    file_tuple = tuple(os.path.join(root, base_filename + ext) for ext in self.extensions)
                    ...
                    matching_files = [os.path.join(root, f) for f in files if os.path.splitext(f)[0] == base_filename]
                    if matching_files:
    ...
    def get_next_shuffled(self)->tuple:
        ...
        if self._shuffle is False:
            raise ValueError('Shuffle is set to False. Cannot get next shuffled file tuple.')
